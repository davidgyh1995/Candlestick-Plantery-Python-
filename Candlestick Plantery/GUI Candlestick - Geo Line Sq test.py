...
a = pd.read_csv('C:/Users/david/OneDrive/Desktop/Candlestick Plantery/test_600539.csv')
...
def Drawstar(Star):
    ...
    if Stardict[Star][1] == 0:
        b = pd.read_csv('C:/Users/david/OneDrive/Desktop/Candlestick Plantery/Data/Geocentric Ephemerides (1900 - 2030).csv')
...
def ShowLine(E1,E2,E3,Combo1,E5,E6,self):
    ...
    global Stardict
    Starindex = Stardict[Star][0]
    b = pd.read_csv('C:/Users/david/OneDrive/Desktop/Candlestick Plantery/Data/Heliocentric Ephemerides (1900 - 2050).csv')
    b['Time'] = pd.to_datetime(b["Time"], format="%m/%d/%Y")
    NewDates = [Date]
    print(DDate)
    print(NewDates)
...
def Sq_Line_Draw (E1,E2,E3,Combo1,E7,E8,E9,E10,self):
    ...
    StartDate_Sq_Line = E1.get() + E2.get() + E3.get()
    EndDate_Sq_Line = E7.get()
    ...
    L_or_H = E8.get()
    Scale_Sq_Line = float(E9.get())
    UpOrDown = E10.get()
    #StartDate_Sq_Line = '20050718' #20010904,20050718,20070523,20081016
    StarDatetime_Sq_Line = datetime.strptime(StartDate_Sq_Line, '%Y%m%d')
    #EndDate_Sq_Line = '20060718' #20010904,20050718,20070523,20081016
    EndDatetime_Sq_Line = datetime.strptime(EndDate_Sq_Line, '%Y%m%d')


    '''Below section is to search the range of Start and End date, and used the range to creat new list of date and angle'''
    for x_index in range(len(Sq_Line_Xdate)):
        if Sq_Line_Xdate[x_index] == StarDatetime_Sq_Line:
            Start_index = x_index
        if Sq_Line_Xdate[x_index] == EndDatetime_Sq_Line:
            End_index = x_index

    Sq_Line_Xdate = (a['Date'][Start_index:End_index+1])
    Sq_Line_Yangle = (a[PlanetName][Start_index:End_index+1])



    #Sq_Line_Xdate = Sq_Line_Xdate.values.tolist()
    Sq_Line_Yangle = Sq_Line_Yangle.values.tolist()

    '''Below section is to add 360 to smooth the data, so that the line won't look like sin wave'''
    cycle_Sq_Line = 0
    Before_Added_Cycle = Sq_Line_Yangle[0]

    print(Sq_Line_Yangle)
    ...
    print(Sq_Line_Yangle)
    '''panda csv a has both the stock data and angle data combined. Off_Set is a list contain only 1 number of low of high price data from start index'''
    Off_Set = [a[L_or_H][Start_index].tolist()]

    New_Yangle = Off_Set
    print("new yangle out of loop: ",New_Yangle)
    '''Up or down trend line. '''
    if UpOrDown == "Up":
        for o_index in range(len(Sq_Line_Yangle)-1):
            New_Yangle = New_Yangle + [(Sq_Line_Yangle[o_index+1] - Sq_Line_Yangle[0]) *Scale_Sq_Line + float(Off_Set[0])]
        print("new yangle in loop: ", New_Yangle)
    else:
        for o_index in range(len(Sq_Line_Yangle)-1):
            New_Yangle = New_Yangle + [(Sq_Line_Yangle[0] - Sq_Line_Yangle[o_index+1]) *Scale_Sq_Line + float(Off_Set[0])]


    GeoPlt = plt.plot(Sq_Line_Xdate,New_Yangle,color = Stardict[PlanetName][2])
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Combo1.set("Mars")
        Combo1.place(x=60, y=50,relwidth=0.055, relheight=0.025)
...
############################################################################
        label7 = tk.Label(self, text="EndDate: ", font=LARGE_FONT)
        label7.place(x=700, y=0)

        E7 = tk.Entry(self)
        E7.insert(tk.END, "20080718")  
        E7.place(x=780, y=0,relwidth=0.04, relheight=0.02)

        label8 = tk.Label(self, text="High/Low: ", font=LARGE_FONT)
        label8.place(x=360, y=0)

        '''E8 = tk.Entry(self)
        E8.insert(tk.END, "Low")  
        E8.place(x=450, y=0,relwidth=0.02, relheight=0.02)'''

        E8 = ttk.Combobox(self, values=("Low",
                                        "High"), font=LARGE_FONT)
        E8.set("Low")
        E8.place(x=450, y=0,relwidth=0.02, relheight=0.02)

        label9 = tk.Label(self, text="Scale: ", font=LARGE_FONT)
        label9.place(x=360, y=50)

        E9 = tk.Entry(self)
        E9.insert(tk.END, "0.01")  
        E9.place(x=420, y=50,relwidth=0.02, relheight=0.02)
        
        labe20 = tk.Label(self, text="Up/Down: ", font=LARGE_FONT)
        labe20.place(x=450, y=50)

        E10 = ttk.Combobox(self, values=("Down",
                                        "Up"), font=LARGE_FONT)
        E10.set("Up")
        E10.place(x=500, y=50,relwidth=0.02, relheight=0.02)

        '''E10 = tk.Entry(self)
        E10.insert(tk.END, "u")  
        E10.place(x=480, y=50,relwidth=0.02, relheight=0.02)'''
...
###########
        ...
        self.button277 = tk.Button(self, text="Test Sq Line",
                            command=lambda: Sq_Line_Draw(E1,E2,E3,Combo1,E7,E8,E9,E10,self))
        self.button277.place(x=700, y=40,relwidth=0.06,relheight=0.03)
